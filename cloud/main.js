...
//	Message is not specified
//	Countries is not specified
Parse.Cloud.define('sfInventoryUpdate', function(request, response) {
	
	if (!_.has(request.params, 'product') || _.isEmpty(request.params.product))
		return response.error('Product is not specified');
	...
	Parse.Promise.as().then(
		...
		function () {
			...
			return request.master === true ? Parse.Promise.as(true) : checkUserRole(request.user, ROLE_ADMIN);
		...
		function () {
			...
			return salesforce.getConnection().then(
		...
		function () {
			...
			query.exists('sizes');
			return query.get(request.params.product, request.user instanceof Parse.User ? {sessionToken: request.user.getSessionToken()} : {});
		...
		function (product) {
			...
			var inventory = salesforce.createInventoryFromProduct(product);
			
			return conn.sobject("Inventory__c").upsert(inventory, 'ExtId__c');
...
Parse.Cloud.define('getAccounts', function(request, response) {
	...
	Parse.Promise.as().then( 
		function () {
			...
			query.include(['defaultShippingAddress','referrer','productWishList', 'group']);
			query.exists('isUpdated');
			return query.find(request.user instanceof Parse.User ? {sessionToken: request.user.getSessionToken()} : {});
			.then(
				null,
				function() {
...
// sf Account Update
Parse.Cloud.define('sfAccountUpdate', function(request, status) {
	
	if (!_.has(request.params, 'account') || _.isEmpty(request.params.account))
		return response.error('Account is not specified');
	...
	Parse.Promise.as().then(
		...
		function () {
			...
			query.include(['defaultShippingAddress','referrer','productWishList', 'group']);
			query.exists('isUpdated');
			return query.get(request.params.account, request.user instanceof Parse.User ? {sessionToken: request.user.getSessionToken()} : {});
		...
		function (user) {
			...
			var account = salesforce.createAccountFromUser(user);
			
			return conn.sobject("Account").upsert(account, 'ExtId__c');
...
Parse.Cloud.job('sfInventoryUpdate', function(request, status) {
	...
	query.each(
		...
		function (product) {
			...
			return Parse.Cloud.run('sfInventoryUpdate', {product: product.id}, {useMasterKey: true}).then(
		...
		function () {
			status.success('Total - ' + totalCount + ', success - ' + successCount);
		},
		function (error) {
